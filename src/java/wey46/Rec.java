...
public class Rec {
    ...
    public static ArrayList<Movie> getRec(ArrayList<Rating> list, int howmany) throws SQLException{
        ...
        String sql = "SELECT Name,ReleaseDate as yr,Duration,PosterUrlHori,PosterUrlVert,Overview FROM Movies WHERE MovieId = ?";
	ResultSet rs = null;
        try (
	    ...
	    PreparedStatement stmt = conn.prepareStatement(sql);
	        ...
	        if (rs.next()) {
		    Movie m = new Movie(movieId, rs.getString("Name"), rs.getString("yr"), rs.getString("Overview"),
